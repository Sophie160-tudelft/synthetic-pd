...
R1 = home/living-room baseline with table
R2 = second home baseline / stable frontal setup without table
...
2. R2 - R1: difference between the two home baseline setups
3. R3 - R2: camera-view effect, upper corner versus the R2 home baseline
4. R4 - R1: severe lower-body occlusion versus the R1 home baseline
5. R5 - R1: partial frontal occlusion versus the R1 home baseline
...
# Extra-subject reduced outputs are often saved by script 06 as E0--E5,
# even though the thesis interprets them as matched full-matrix conditions.
# Example for SUB02/SUB08/SUB17:
# output E0 -> R0 -> source E0
# output E1 -> R1 -> source E1
# output E2 -> R2 -> source E3
# output E3 -> R3 -> source E4
# output E4 -> R4 -> source E8
# output E5 -> R5 -> source E11
REPLICATION_OUTPUT_TO_REDUCED = {
    "E0": "R0",
    "E1": "R1",
    "E2": "R2",
    "E3": "R3",
...
DEFAULT_CONDITION_ROWS = [
    ...
    },
    {
        "reduced_id": "R2",
        "original_id": "E3",
        "purpose": "Home baseline: stable frontal setup",
        "environment": "Living room without table",
        "lighting": "Bright/day",
        "camera_view": "Frontal/table 0.95 m",
        "occlusion": "None",
        "why_include": "Second home baseline and camera-view reference",
    },
    {
        "reduced_id": "R3",
        "original_id": "E4",
...
DEFAULT_COMPARISON_ROWS = [
    ...
    },
    {
        ...
        "test_id": "R2",
        "baseline_id": "R1",
        "comparison_name": "Stable frontal home baseline vs home baseline with table",
        "question_answered": "How different are the two home baseline setups?",
    },
    {
        "comparison_id": "C3",
        "test_id": "R3",
        "baseline_id": "R2",
        "comparison_name": "Upper-corner view vs R2 home baseline",
        "question_answered": "Does camera viewpoint affect recovered motion relative to the R2 home baseline?",
    },
    {
        "comparison_id": "C4",
        "test_id": "R4",
        "baseline_id": "R1",
        "comparison_name": "Strong occlusion vs R1 home baseline",
        "question_answered": "How much degradation is introduced by severe lower-body occlusion relative to the R1 home baseline?",
    },
    {
        "comparison_id": "C5",
        "test_id": "R5",
        "baseline_id": "R1",
        "comparison_name": "Partial frontal occlusion vs R1 home baseline",
        "question_answered": "Does partial one-leg occlusion remain recoverable relative to the R1 home baseline?",
    },
    {
        "comparison_id": "C6",
        "test_id": "R5",
...
def load_condition_map(condition_map_csv: Optional[Path], output_dir: Path) -> pd.DataFrame:
    ...
    if "original_id" not in df.columns:
        df["original_id"] = df["reduced_id"].astype(str).map(REDUCED_TO_SOURCE_FULL_MATRIX).fillna("unknown")
...
def detect_id_layout(experiment_ids: Sequence[str]) -> str:
    """
    Detect whether the summary file uses:
    - reduced labels R0--R5,
    - full-matrix source labels E0,E1,E3,E4,E8,E11,
    - replication output labels E0--E5.
    """
    ids = {str(x) for x in experiment_ids}

    if {f"R{i}" for i in range(6)}.issubset(ids):
        return "reduced_R0_to_R5"

    # Primary/full-matrix-style IDs. SUB01 may also contain E2 and E5, but the
    # presence of E8/E11 means E4 should remain R3 and E8/E11 should become R4/R5.
    if {"E0", "E1", "E3", "E4", "E8", "E11"}.issubset(ids):
        return "full_matrix_source_ids"

    # Replication reduced outputs from script 06 are commonly stored as E0--E5.
    # In that layout E4 is NOT the upper-corner source condition; it is output R4.
    if {"E0", "E1", "E2", "E3", "E4", "E5"}.issubset(ids):
        return "replication_output_E0_to_E5"
...
def add_reduced_ids(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    df["output_id"] = df["experiment"]

    layout = detect_id_layout(df["experiment"].unique())

    if layout == "reduced_R0_to_R5":
        df["reduced_id"] = df["experiment"]
    elif layout == "replication_output_E0_to_E5":
        df["reduced_id"] = df["experiment"].map(REPLICATION_OUTPUT_TO_REDUCED)
    elif layout == "full_matrix_source_ids":
        df["reduced_id"] = df["experiment"].map(ORIGINAL_TO_REDUCED)
    else:
        # Fallback: try both mappings. This keeps the script usable if a partial
        # reduced matrix is present, but the printed warning makes the layout clear.
        print(
            "WARNING: Could not confidently detect ID layout. "
            "Trying full-matrix mapping first and replication E0--E5 mapping second."
        )
        df["reduced_id"] = (
            df["experiment"]
            .map(ORIGINAL_TO_REDUCED)
            .fillna(df["experiment"].map(REPLICATION_OUTPUT_TO_REDUCED))
        )

    df["original_id"] = df["reduced_id"].map(REDUCED_TO_SOURCE_FULL_MATRIX)
    df["id_layout"] = layout
    ...
    if reduced.empty:
        raise ValueError(
            "No reduced-matrix experiments found. Expected either R0-R5, "
            "full-matrix IDs E0,E1,E3,E4,E8,E11, or replication output IDs E0-E5."
        )

    found = set(reduced["reduced_id"].astype(str))
    missing = sorted(valid_reduced - found, key=id_number)
    if missing:
        print(f"WARNING: Missing reduced conditions: {', '.join(missing)}")

    print(f"ID layout detected: {layout}")
    print("Reduced-condition mapping used:")
    mapping_cols = ["reduced_id", "output_id", "original_id"]
    print(sort_by_reduced_id(reduced[mapping_cols].drop_duplicates()).to_string(index=False))
...
def make_key_results_table(summary: pd.DataFrame) -> pd.DataFrame:
    keep = [
        "reduced_id", "output_id", "original_id", "purpose", "environment", "lighting", "camera_view", "occlusion",
...
def find_per_frame_csv(row: pd.Series, subject_out_root: Path) -> Optional[Path]:
    ...
    candidates_roots = []
    for key in ["experiment", "output_id", "reduced_id", "original_id"]:
        if key in row:
    ...
    search_tokens = [str(row.get("experiment", "")), str(row.get("output_id", "")), str(row.get("reduced_id", "")), str(row.get("original_id", ""))]

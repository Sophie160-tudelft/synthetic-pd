...
def plot_frame_delta_vs_home(metrics_base: Path, figs_dir: Path) -> None:
    ...
    set_common_axes(ax, "Frame-specific pose degradation relative to the E1 home baseline", xlabel="Frame", ylabel=r"$\Delta$ PA-MPJPE relative to E1 (mm)")
    ax.legend(fontsize=8)
    fig.tight_layout()
...
def make_visual_examples(figs_dir: Path, visual_examples: Optional[Path], render_frames_root: Optional[Path]) -> str:
    ...
    note = (
        ...
        "The script will sample the peak/error frames for E3, E8, E11, E4, and E10."
    )
    ...
    examples = [
        ...
        ("E11 partial occlusion", "SUB01", "SUB01_off_walk_1", "E11", 323),
